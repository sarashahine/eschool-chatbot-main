...
from config import ARABIC_COLLECTION_NAME, NORMALIZE_EMBEDDINGS

arabic_chunk_routes = Blueprint("arabic_chunk_routes", __name__)

# -----------------------------
# Insert chunk
# -----------------------------
@arabic_chunk_routes.route("/chunks", methods=["POST"])
def insert_chunk():
    try:
        ...
        vector = current_app.embedder.encode(text, normalize_embeddings=NORMALIZE_EMBEDDINGS,convert_to_tensor=False).tolist()
        payload = {
            "text": text,
...
# Get a single chunk
# -----------------------------
@arabic_chunk_routes.route("/chunks/<int:chunk_id>", methods=["GET"])
...
# Update a chunk
# -----------------------------
@arabic_chunk_routes.route("/chunks/<int:chunk_id>", methods=["PUT"])
def update_chunk(chunk_id):
    try:
        ...
        if new_text:
            ...
            vector = current_app.embedder.encode(new_text, normalize_embeddings=NORMALIZE_EMBEDDINGS).tolist()
        ...
        current_app.qdrant_client.upsert(collection_name=ARABIC_COLLECTION_NAME, points=[updated_point], wait=True)
...
# Delete a chunk
# -----------------------------
@arabic_chunk_routes.route("/chunks/<int:chunk_id>", methods=["DELETE"])
